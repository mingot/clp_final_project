...
\\
\\
En la figura \ref{fig:nn_penalty} podem observar com varia l'error en el conjunt de test (mitja dels $k$ entrenaments) en funció del terme de regularització. En totes elles estem usant 8. Per tant ens quedem amb un valor de X per aquest terme. Usant aquest valor veiem que 

[Nombre de capes ocultes i nombre de variables en capa]
...
\bibitme{hastie09} % neural networks
